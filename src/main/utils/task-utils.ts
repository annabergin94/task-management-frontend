...
export const getTaskActionsHtml = (taskId: number): string => `
  <div style="display: inline;">
    ...
    <form method="POST" action="/delete/${taskId}" style="display: inline; margin-left: 10px;">
...
export const mapTasksToTableRows = (tasks: Task[]): TableRow[] =>
  tasks.map((task) => [
    ...
    { text: task.status },
    task.dueDate ? { text: new Date(task.dueDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) } : { text: '' },
